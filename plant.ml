type 'a tree = Literal of string | Binary of 'a * string * 'a
[@@deriving show, eq]

module Expr : Recursion.Functor.S with type 'a t = 'a tree = struct
  type 'a t = 'a tree
...
let f : int ExprR.attr Expr.t -> int = function
  | Literal _ -> 1
  | Binary (lhs, _, rhs) -> max lhs.attribute rhs.attribute + 1
