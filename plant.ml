...
let to_string : (string list * int * int) R.algebra =
 fun plant ->
  ...
  | Root (lines, len, indent) ->
      let line = mk_line len indent ' ' "^" in
  ...
  | Fork
    ...
    ->
      let top_filling nlines len_line =
        List.init nlines (String.make len_line ' ' |> Fun.const)
      in
      let fill_left lines = List.map (fun s -> " " ^ s) lines in
      let fill_right lines = List.map (fun s -> s ^ " ") lines in
      let len, indent = (llen + clen + rlen + 2, llen + cindent + 1) in
      ...
      let llines = llines @ top_filling (nlines - nllines) llen |> fill_right in
      let llen =
        llen + 1
        (* due to filling the llines above *)
      in
      let clines = clines @ top_filling (nlines - nclines) clen in
      let rlines = rlines @ top_filling (nlines - nrlines) rlen |> fill_left in
      let rlen =
        rlen + 1
        (* due to filling the rlines above *)
      in
