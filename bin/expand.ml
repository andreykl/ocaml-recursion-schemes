(** expand.ml — PPX expansion inspector
    ...
    What it does: 1) Reads an OCaml implementation either from a file
    (positional FILE argument) or from stdin (if FILE is omitted). 2) Parses the
    input into a Parsetree using the compiler parser. 3) Runs ppxlib's driver on
    the parsed structure (Driver.map_structure), which applies all registered
    transformations (extensions, derivers, etc.). Important: this works only if
    the executable links in the library/module that registers your deriver (i.e.
    executes Deriving.add at startup; e.g. we add recurion_schemes.ppx to the
    libraries stanza in the dune file). 4) Prints the result for the selected
    AST stage and output view:
    - --stage raw|exp raw : prints the parsed input AST (before running any PPX)
      exp : prints the AST after running ppxlib's driver (Driver.map_structure)
    - --view tree|src tree : prints an AST "tree" dump (like ocamlc
      -dparsetree). For this output we convert ppxlib's selected AST
      representation into the compiler's current Parsetree and print it via
      Ocaml_common.Printast. src : pretty-prints the OCaml code (either before
      or after PPX, depending on --stage).
...
let view_term : view Term.t =
  let doc =
    "Output view mode: $(b,tree) prints Parsetree, $(b,src) prints OCaml \
     source code."
  in
...
let stage_term : stage Term.t =
  let doc =
    "AST stage: $(b,raw) prints the parsed input AST, $(b,exp) prints the AST \
     after PPX expansion."
  in
  let stages = [ ("raw", Raw); ("exp", Expanded) ] in
  Arg.(
    value & opt (enum stages) Expanded & info [ "stage" ] ~docv:"raw|exp" ~doc)
