module Schemes (F : Functor.Functor) = struct
  ...
  let rec apo (f : 'a -> ('b term, 'a) Either.t F.t) (a : 'a) : 'b term =
    let funin = Either.fold ~left:Fun.id ~right:(apo f)
    in f a |> F.map funin |> (fun x -> W x)

  let rec histo (f : 'a cvalgebra) (W x : 'b term) : 'a =
    let rec worker (W x') = {
        attribute = histo f (W x') ;
        hole = F.map worker x'
      }
    in F.map worker x |> f
  ...
  let para'' (f : 'b term -> 'a F.t -> 'a) (bt : 'b term) : 'a =
    histo' (fun aaf ->
        ...
        let rec worker aaf = W (F.map (fun { hole = aaf ; _ } -> worker aaf) aaf) in
